...
現在は段階 4（テキスト・付箋・図形のラベル、IME 対応）まで。ほかに、Markdown カードを画像にして描く方法の検証ページ（`?lab=markdown`）と、試作版の Markdown カードがある。データはまだ保存されず、再読み込みで消える。
...
| 全体表示 | Shift+1 |
| ツール | V 選択 / H 手のひら / R 矩形 / O 楕円 / T テキスト / N 付箋 |
