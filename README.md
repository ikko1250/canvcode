...
現在は段階 8（矢印）まで。ほかに、Markdown カードを画像にして描く方法の検証ページ（`?lab=markdown`）と、試作版の Markdown カードがある。ノードはまだ保存されず、再読み込みで消える（貼り付けた画像のファイルだけは、サーバーのワークスペースに保存される）。
...
| `packages/nodes` | ノードの型（`defineNodeType`、図形・テキスト・付箋・画像・フリーハンド・矢印・グループ・フレーム） |
| `packages/canvas` | 描画（Canvas2D の 3 レイヤー）・入力・ツール・空間インデックス・矢印のつながり |
| `apps/web` | React の画面 |
...
| 全体表示 | Shift+1 |
| ツール | V 選択 / H 手のひら / R 矩形 / O 楕円 / T テキスト / N 付箋 / F フレーム / D フリーハンド / E 消しゴム / A 矢印 |
