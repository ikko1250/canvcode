...
現在は段階 7（フリーハンドと消しゴム）まで。ほかに、Markdown カードを画像にして描く方法の検証ページ（`?lab=markdown`）と、試作版の Markdown カードがある。ノードはまだ保存されず、再読み込みで消える（貼り付けた画像のファイルだけは、サーバーのワークスペースに保存される）。
...
| `packages/nodes` | ノードの型（`defineNodeType`、図形・テキスト・付箋・画像・フリーハンド・グループ・フレーム） |
...
| 全体表示 | Shift+1 |
| ツール | V 選択 / H 手のひら / R 矩形 / O 楕円 / T テキスト / N 付箋 / F フレーム / D フリーハンド / E 消しゴム |
